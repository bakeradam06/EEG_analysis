...
filtData2.df <- allData.df %>%
  ...
  filter(Condition == "NoVib",
         Phase == "Exe",
         BrainRegion %in% c('PML','M1L','SML','S1L'),
         ) %>%
    summarise(meanCoh = mean(Coh), .groups = 'drop')

NHPTdistinct <- allData.df %>%
  distinct(Subject, NHPTafffected, session)

filtData2.df <- left_join(filtData2.df,
                           NHPTdistinct %>% select(Subject, NHPTafffected),
                           by = "Subject")




testBandMuscRegion_glm <- glm(meanCoh ~
                  # fixed effects
                  NHPTafffected +
                  Band +
                  Muscle +
