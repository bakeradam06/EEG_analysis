...
hist(unique(log(test2.df$ARATscore)),breaks=15)
hist(unique(test2.df$ARATscore),breaks=15)
##################
```
...
m1lApbRec2.df <- groupDataLong.df %>%
  filter(Condition == "NoVib", Band == "Gamma", Phase == "Prep", connection == "PML-APB")

m1lApbRec2 <- lmer(ARATscore ~
              # fixed effects
              meanCoh*session +
              (1 | Subject),
              data = m1lApbRec2.df
              )
Anova(m1lApbRec2, type = 'III')
model_parameters((m1lApbRec2))
check_model(m1lApbRec2)
check_heteroscedasticity(m1lApbRec2)
check_collinearity(m1lApbRec2)
r2(m1lApbRec2)
