...
################################################################################
####### CMC analysis, potentially for dissertation, conference abstract ########
################################################################################
...
filtData2.df <- allData.df %>%
  ...
  filter(Condition == "NoVib",
         Band == "Beta", # change to beta if desired
         Phase == "Exe",
...
temp_filtData2.df <- filtData2.df %>%
  #filter(connection %in% c("PML-FDI","PML-EDC","M1L-FDI","M1L-EDC"),
  filter(connection %in% c("PML-EDC"), # For ASNR 2026 abstract poster, change between "M1L-EDC" and "PML-EDC", then run two models below.
         session %in% c("Baseline"))
# model - NHPT 
modGamExeNHPTa <-  glm(NHPTafffected ~ 
                       meanCoh, 
                       data = temp_filtData2.df
                       )
Anova(modGamExeNHPTa, type = 'III')
summary(modGamExeNHPTa)
model_parameters(modGamExeNHPTa) # BETA: "PML-EDC": p<0.001; "PML-FDI": p=0.653, "M1L-FDI": p=0.799; M1L-EDC: p=0.002
check_heteroscedasticity(modGamExeNHPTa)
check_residuals(modGamExeNHPTa)
report(modGamExeNHPTa)

gam1 <- gam(data = temp_filtData2.df,
            formula = NHPTafffected ~ s(meanCoh))
summary(gam1)
model_parameters(gam1)
check_model(gam1)
plot.Gam(gam1, se=T)

spearman_result <- cor.test(temp_filtData2.df$NHPTafffected, temp_filtData2.df$meanCoh, 
                            method = "spearman", data=temp_filtData2.df)
spearman_result$statistic
spearman_result$p.value
spearman_result$estimate

# model - BBT
modGamExeBBTa <-  glm(BBTaffected ~ 
                       meanCoh,
                       data = temp_filtData2.df
                       )

Anova(modGamExeBBTa, type = 'III')
summary(modGamExeBBTa)
model_parameters(modGamExeBBTa) # "PML-EDC": p=0.004; "PML-FDI": p=0.169, "M1L-FDI": p=0.190; M1L-EDC: p=0.235
report(modGamExeBBTa)
check_heteroscedasticity(modGamExeBBTa)
check_residuals(modGamExeBBTa)
