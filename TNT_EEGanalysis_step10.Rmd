...
```
Second set of models. PML and motor recovery. Same research question as above, just different
  modeling strategy
```{r}
# prep dataset
...
filtDataGamExe2.df <- filtDataGamExe.df %>%
  filter(connection %in% c("PML-APB","PML-FDI","PML-FDS","PML-EDC",
                           "M1L-APB","M1L-FDI","M1L-FDS","M1L-EDC"))

filtDataGamExe2.df %>%
  filter(session == 'Baseline') %>%
  ggplot(aes(x=BBTaffected, y=meanCoh)) +
    geom_point() +
...
test.df <- filtDataGamExe.df %>%
  filter(connection == 'M1L-FDS',
         session == 'Baseline')
cor(test.df$meanCoh,test.df$BBTaffected)
```
...
filtData2.df <- allData.df %>%
  ...
  filter(Condition == "NoVib",
         Phase == "Exe",
         Band == "Beta",
         connection %in% c("PML-APB","M1L-APB")
         )

filtData3.df <- filtData2.df %>%
  group_by(Subject, session, connection) %>%
  summarise(meanCoh = mean(Coh))
  
filtData2.df <- right_join(filtData3.df, 
                           demogData.df, by=c("Subject","session"))

filtData3_deriv1.df <- filtData3.df %>%
  filter(session=="Baseline")
...
testBandMuscRegion_glm <- glm(NHPTafffected ~
                  meanCoh +
                  connection,
                  data = filtData3Test.df
                  )

Anova(testBandMuscRegion_glm, type = 'III')

testBandMuscRegion_lmer <- lmer(Coh ~
                  # fixed effects
                  NHPTafffected +
                  connection +
                  # random effects 
...
Anova(testBandMuscRegion_lmer, type = 'III')
anova(testBandMuscRegion_step,testBandMuscRegion_lmer)

Anova(testBandMuscRegion_glm, type = 'III')
# Anova(testBandMuscRegion_lmer, type = 'III')
anova(testBandMuscRegion_glm)
summary(testBandMuscRegion_glm)
model_parameters(testBandMuscRegion_glm, ci_method = "wald")
# model_parameters(testBandMuscRegion_lmer, ci_method = "wald")
# check_model(pmlApbRec)
check_heteroscedasticity(testBandMuscRegion_glm)
check_collinearity(testBandMuscRegion_glm)
r2(testBandMuscRegion_glm)
# report(testBandMuscle)


#################################################
##################### OBSOLETE ##################
######### same model but random slope ###########
#################################################
testBandMuscRegion2 <- lmer(Coh ~
                  # fixed effects
                  NHPTafffected +
                  BBTaffected + 
                  ARATscore +
...
Anova(testBandMuscRegion2, type = 'III')
model_parameters((testBandMuscRegion2))
# check_model(pmlApbRec)
check_heteroscedasticity(testBandMuscRegion2)
check_collinearity(testBandMuscRegion2)
r2(testBandMuscRegion2)

anova(testBandMuscRegion, testBandMuscRegion2)
...
testBandMuscRegion_step <- step(testBandMuscRegion, direction = 'backward')
finalBandMuscRegModel <- get_model(testBandMuscRegion_step)

Anova(finalBandMuscRegModel, type = 'III')
model_parameters((finalBandMuscRegModel))
check_heteroscedasticity(finalBandMuscRegModel)
check_collinearity(finalBandMuscRegModel)
r2(finalBandMuscRegModel)
# x <- check_model(finalBandMuscRegModel)


## back elim with random slope
testBandMuscRegionRand_step <- step(testBandMuscRegion2)
finalBandMuscRegModel_Rand <- get_model(testBandMuscRegionRand_step)

Anova(finalBandMuscRegModel_Rand, type = 'III')
model_parameters((finalBandMuscRegModel_Rand))
check_heteroscedasticity(finalBandMuscRegModel_Rand)
check_collinearity(finalBandMuscRegModel_Rand)
r2(finalBandMuscRegModel_Rand)


anova(finalBandMuscRegModel,finalBandMuscRegModel_Rand)
