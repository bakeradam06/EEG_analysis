...
filtPML.df <- allData.df %>%
  filter(Condition == "NoVib",
         Phase == "Exe")

testBandMuscRegion <- lmer(Coh ~
                  # fixed effects
                  NHPTafffected +
                  BBTaffected + 
                  ARATscore +
...
Anova(testBandMuscRegion, type = 'III')
model_parameters((testBandMuscRegion))
# check_model(pmlApbRec)
check_heteroscedasticity(testBandMuscRegion)
check_collinearity(testBandMuscRegion)
r2(testBandMuscRegion)
# report(testBandMuscle)

testBandMuscRegion_step <- step(testBandMuscRegion)
...
check_model(finalBandMuscRegModel)
